# Agent Panel Implementation Plan (Phase 2)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an AI chat panel to Zotero with multi-step tool calling — the Agent can search the wiki, read pages, list papers, and trigger ingest.

**Architecture:** Single module `agentPanel.ts` builds the chat UI with DOM APIs and implements the OpenAI function-calling loop via XHR. Tool executors call into `wikiReader.ts` and `ingest.ts`. The panel is registered as a separate `ItemPaneManager` section alongside the Wiki Browser.

**Tech Stack:** Zotero 9 / Firefox ESR 115 sandbox, XPCOM-free (panel rendering only), `wikiReader.ts` data layer, XHR for LLM calls, DOM APIs for UI.
...
| File                           | Action | Purpose                                           |
| ------------------------------ | ------ | ------------------------------------------------- |
| `src/modules/agentPanel.ts`    | Create | Chat UI, tool loop, tool executors, system prompt |
| `src/hooks.ts`                 | Modify | Register Agent panel section                      |
| `addon/locale/en-US/addon.ftl` | Modify | Agent locale strings                              |
| `addon/locale/zh-CN/addon.ftl` | Modify | Agent locale strings                              |

---

### Task 1: Create `src/modules/agentPanel.ts` — chat UI shell

**Files:**

- Create: `src/modules/agentPanel.ts`

- [ ] **Step 1: Write the chat UI with DOM APIs**
...
Expected: TypeScript compiles. There will be warnings about unused imports (searchPages, readPage, etc.) — these are used in later tasks.

- [ ] **Step 3: Commit**

```bash
git add src/modules/agentPanel.ts
git commit -m "feat: add Agent panel chat UI shell"
```

---

### Task 2: `agentPanel.ts` — message rendering + LLM calling with tools

**Files:**

- Modify: `src/modules/agentPanel.ts`

- [ ] **Step 1: Add message rendering, API call, and tool calling loop**

Append after `renderAgentPanel`:
...
```bash
git add src/modules/agentPanel.ts
git commit -m "feat: add Agent LLM calling with tool definitions and send handler"
```

---

### Task 3: `agentPanel.ts` — tool execution + visualization cards

**Files:**

- Modify: `src/modules/agentPanel.ts`

- [ ] **Step 1: Add tool executor and tool card rendering**

Append after `updateSendButton`:
      ...
      case "list_papers": {
        ...
        result =
          papers.length === 0
            ...
            : papers
                .map((p) => `- (${p.year}) **${p.title}** — ${p.summary}`)
                .join("\n");
        break;
      }
      case "ingest_selected": {
        // @ts-expect-error - Zotero globals
...
function addToolCard(name: string, args: Record<string, unknown>): ToolCard {
  if (!state.chatEl || !state.doc) {
    return { el: state.doc!.createElement("div"), update: () => {} };
  }
  const doc = state.doc;
...
```bash
git add src/modules/agentPanel.ts
git commit -m "feat: add Agent tool execution and visualization cards"
```

---

### Task 4: Register Agent panel in `hooks.ts` + locale strings
...
# Agent 面板
section-agent-head-text = Agent
section-agent-sidenav-tooltip = 与 AI 研究助手对话
```

- [ ] **Step 2: Add import and registerSection in hooks.ts**

Read `src/hooks.ts`. Add import after the wikiBrowser import:

```typescript
import { renderAgentPanel } from "./modules/agentPanel";
```

After the wikiBrowser registerSection block, add:
...
- [ ] **Step 3: Build and verify locale types**

```bash
npm run build
grep "section-agent" typings/i10n.d.ts
```

Expected: New Fluent message IDs appear.

- [ ] **Step 4: Commit**

```bash
git add src/hooks.ts addon/locale/en-US/addon.ftl addon/locale/zh-CN/addon.ftl
git commit -m "feat: register Agent panel in ItemPaneManager"
```

---

### Task 5: End-to-end build and smoke test
...
Expected: Zero errors, output at `.scaffold/build/addon/content/scripts/llmwiki.js`.
...
1. Zotero right pane shows "Agent" tab
2. Click "Agent" tab — welcome message and input area visible
3. Type "list my papers" and press Enter — Agent calls list_papers tool → shows tool card → displays paper list
4. Type a question about a specific topic — Agent searches wiki and responds
5. Verify tool cards are collapsible (click to expand/collapse)
6. Verify Shift+Enter produces newline in input (doesn't send)
7. Select a paper in Zotero, type "compile this paper" — Agent should run ingest

- [ ] **Step 3: Commit any final fixes**

```bash
git add -A
git commit -m "chore: final polish for Agent panel"
```
